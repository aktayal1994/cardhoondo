...
## Hero rebuilt around user-supplied photography, deployed (Aug 22, same day)

Went through two intermediate versions before landing on what actually shipped, both superseded same-session (no separate commits for them, to avoid documenting states that were never live): first an abstract "radar/signal" illustration (replacing an earlier hand-drawn one the user found "childish"), then a version that overlaid the real user-supplied hero photos with a duplicate HTML headline + dark scrim for legibility.

**Final, shipped version**: the user clarified the photos already have the full headline ("Asked chacha... Still confused which car to buy?") baked into the artwork itself, and gave a detailed spec -- don't duplicate that text in HTML, don't crop the compositions to fill a fixed box, use real `<picture>`-style breakpoint switching around 768px, keep the CTA off the photo rather than overlaid.

- Two `next/image` elements (desktop landscape / mobile portrait -- genuinely different compositions, not one image cropped) rendered at intrinsic aspect ratio (`width`/`height` + `h-auto w-full`, no `fill`/`object-cover`), swapped via `hidden md:block` / `md:hidden` at Tailwind's `md` (768px) breakpoint -- verified the actual rendered image dimensions match the source aspect ratio exactly at both breakpoints, confirming zero cropping.
- Alt text now carries the full headline message (`"Asked chacha. Asked colleagues. ... A couple stands on a coastal road as CarDhoondo highlights one clear, confidently recommended car."`) since it's no longer duplicated anywhere in the DOM -- this is the only accessible copy of that message for screen readers.
- CTA moved off the photo entirely into a dark "stage" band directly below the image (`Find My Car` + `13 questions · ~3 minutes · No signup required`, matching the user's exact requested copy) -- this also fully retires the contrast-verification problem from the two earlier versions, since there's no text sitting on top of photo pixels anymore.
- A small brand kicker (icon + "Built for car buyers in India") was added above the image in this pass, then removed one round later per direct user feedback pointing at exactly that element in a screenshot -- gone entirely now, image sits directly under the nav.
- Verified live on `cardhoondo.com` post-deploy (console clean, correct image active, no duplicated headline in the page text) before pushing here.

## Landing page redesign: Airbnb-referenced "anti-slop" pass + real motion, deployed (Sep 5-6)

Two `.claude/skills/` were added to the internal project this session (not part of this public repo): `taste-skill` (a third-party frontend "anti-slop" design skill -- brief inference, forced dial values for variance/motion/density, a long list of banned AI-tell patterns) and `design-md-airbnb` (a plain design-token reference extracted from Airbnb's real site -- colors, type scale, shape/shadow system). Both reviewed file-by-file before installing (plain markdown/YAML, no scripts) given they're third-party downloads. User asked to apply both to `LandingScreen.tsx`.

**Redesign, not a re-skin:**
- **Single light theme, page-wide.** The old page alternated light -> dark (Why CarDhoondo) -> light -> dark (Footer) three times -- exactly the "walked into a different website mid-scroll" tell the skill bans, and inconsistent with Airbnb's own real site (no dark mode on its public pages). Converted Why-CarDhoondo and the footer to light; the hero's dark band stays as the one deliberate exception, since it's a direct continuation of the photo's own legibility scrim, not a separate stylistic flip.
- **Fixed a real 3-equal-card violation** in the trust bar (three identical icon cards side by side is the specific pattern the skill calls out as the most common AI tell) -- replaced with an asymmetric layout: one real stat (`4,036`, the actual review-claim count from the data pipeline) beside a divided list of the three points.
- **New evidence-preview section** -- a real component preview (verdict badge, confidence sentence, quote), not a fake screenshot, built from the project's actual Kia Seltos `suspension_softness` claim data (0.818 score, 11 reviews, high confidence).
- Unified 6 different CTA labels down to one (`Find my car`), removed all eyebrow labels (was 4 across ~7 sections, over the skill's stated budget), and eliminated 20 em-dashes/en-dashes across the page copy and SEO metadata.
- Kept the hero photo, its crop math, and its exact ad-campaign headline copy untouched -- that's live Instagram creative, not something a design pass should silently rewrite.

**Then a second pass added real motion** (`motion` and `gsap` newly added to `package.json`), per a direct follow-up ("stop looking like an AI template, go berserk on animation"): floating "opinion cloud" chips over the hero photo (a literal dramatization of the headline's own "too many conflicting opinions" story, positioned to avoid both the headline text and the photo's documented crop-safe zone), magnetic cursor-pull CTAs, a count-up-on-scroll stat number, a 3D cursor-tilt on the evidence card, a real-cars marquee (16 actual catalogued car names), scroll-reveal lifts, and a GSAP sticky-stack for the "How it works" steps.

**Real bug in the sticky-stack, caught by the user live, fixed in two attempts:**
1. First implementation pinned each step until the *last* step arrived (not the next one) -- so by step 4, steps 1-3 were all still pinned on screen simultaneously, unreadable overlap. User caught this from actually scrolling the page.
2. First fix attempt (chaining each card's un-pin point to the next sibling instead of the last one) never actually activated any pin at all -- verified live that every card's computed `position` stayed `"relative"` through the whole scroll range.
3. Rebuilt with a more robust, standard pattern instead: one `ScrollTrigger` pins the whole container once; all 4 steps sit absolutely stacked on the same rect; opacity for each is a simple tent function (`max(0, 1 - |progress - stepIndex|)`) driven off one `onUpdate` callback, so each step peaks at exactly its own turn with no chained triggers to go wrong. Confirmed structurally (one pin-spacer, correctly sized to a 3-viewport-height runway) -- full live rAF-driven scroll verification was blocked this session by the Browser pane being backgrounded (which throttles `requestAnimationFrame`, the same mechanism GSAP's ScrollTrigger runs on), so the final crossfade feel should get a real look post-deploy.

Verified before deploying: clean `next build` (not just typecheck), and post-deploy against the live domain -- homepage 200, zero em-dashes in the served HTML, new section copy (`Real review claims`, `Chacha says diesel`) present, unrelated routes (`/questionnaire/intro`) unaffected.

## Second live trust stat: videos analyzed, animating alongside claims (Sep 16)

User asked for a second live number next to the existing claims count in the trust section -- "video counts analyzed" -- reasoning it's a real, independently-checkable trust signal (evidence comes from watched review videos, not just extracted claim lines), and wanted both numbers to count up as an animation, not appear static.

`review_claims.video_id` already carries this per claim (see `supabase/schema.sql`, not part of this public repo), including one legacy field reuse: the single manually-uploaded Team-BHP forum thread stores its thread id in the same `video_id` column (documented next to `post_id` in that table). A distinct count therefore needed to exclude `post_id is not null` rows to stay an honest "real videos" number, not videos-plus-one-forum-thread. PostgREST/supabase-js has no `COUNT(DISTINCT ...)` on the query builder, so added a small SQL function, `count_analyzed_videos()`, to the internal project's `supabase/schema.sql` and applied it to the live database via the existing `scripts/apply_schema.py` (idempotent, additive-only -- same low-risk pattern already used for the `respondents` table). Confirmed directly against the live DB before wiring the API: 558 distinct real video ids among 9,341 total claim rows across 81 cars.

- `app/api/stats/route.ts` now calls `supabase.rpc("count_analyzed_videos")` alongside the two existing `head: true` count queries and returns `{ claims, cars, videos }`. Still `dynamic = "force-dynamic"` with `Cache-Control: no-store` -- same "never stale, never cached" contract as the claims/cars counts.
- `components/LandingScreen.tsx` -- `SiteStats`/`FALLBACK_STATS`/`useLiveStats` extended with `videos` (fallback `558`, an exact snapshot at write time, same spirit as the original `4036` claims fallback: a floor shown briefly before the live fetch resolves, never the source of truth). `TrustAndStats`'s single "Case file no. 001" stat card became two side-by-side cards (`StatCard`, a small extracted component) inside a `sm:grid-cols-2` block: claims stays "Case file no. 001", the new videos count is "Case file no. 002" ("Review videos watched, start to finish"), both driven by the page's existing `CountUpNumber` (scroll-triggered, `prefers-reduced-motion`-aware) -- no new animation primitive needed, the existing one just got a second caller.
- Verified live via `npm run dev` in the Claude Browser tool, not just typechecked: `/api/stats` returned the real `{"claims":9341,"cars":81,"videos":558}` from production Supabase; a screenshot mid-scroll caught the new card's count-up actively in flight (184 -> 558, not a static snap); both cards render correctly side by side on desktop and stacked with no overflow at 375px mobile width. `npm run typecheck` and a full `npm run build` both clean.

## Evidence preview: added a real negative example alongside the positive one (Sep 16, same day)

Feedback: the "This is what evidence-backed actually looks like" section (`EvidencePreview`) only ever showed a positive example (Kia Seltos ride quality, `+0.818`, 11 reviews) -- one-sided, and not actually proof the system surfaces real flaws, not just flattering ones.

Searched every car's aggregated claims (`data/claims/*_aggregated.json`, not part of this public repo) for a real, well-corroborated negative facet to match the positive example's rigor (high confidence, double-digit claim count, not a 1-2-claim outlier) -- landed on **Tata Sierra, `fit_finish_quality`: -0.778 score, 17 claims, high confidence**, sourced from a real Autocar India expert review (`mFJgqAwZoPA`). Picked the single cleanest quotable sentence from that facet's evidence list (a steering-wheel-alignment complaint) over a couple of alternatives that either code-switched into Hindi or referenced the Sierra EV in a way that would've read oddly under a plain "Tata Sierra" label -- confirmed the exact quote text, `car_id`, `theme`/`facet`, and `source_type: expert` directly against the raw per-video claim file before using it, not just trusting the aggregated summary.

- Extracted the shared card markup into `EvidenceCardExample` (car, facet label, verdict, sentiment, confidence line, quote, stagger delay) so the positive and negative cards are the same component rendered twice, not two near-duplicate blocks to keep in sync by hand.
- Negative cards get a red circular "X" badge (reusing the existing `X` icon already imported for the FAQ chevrons) in place of the positive card's checkmark `VerdictStamp`, plus the page's existing `negative`/`negative-bg` color tokens (already used elsewhere, e.g. the "usual way" comparison card) for the dot, quote-box background, and quote-mark icon -- no new tokens added.
- Right column went from one card to a `flex-col gap-6` stack of two (positive first, negative second at `delay=0.1` so it settles just after), and the section's grid ratio widened slightly (`1fr/1fr` -> `0.85fr/1.15fr`) to give the taller stack room; intro copy updated to say the evidence includes "a car's real flaws, stated as plainly as its strengths -- not a mystery score, and not just the flattering half."
- Verified live via `npm run dev` in the Claude Browser tool: both cards render with the real quote text and correct verdict/confidence copy, no horizontal overflow at 1440px or 375px (`scrollWidth === clientWidth` at both), and a mobile screenshot confirms clean stacking with the red X badge clear of the outer card edge. `npm run typecheck` and a full `npm run build` (run only after stopping the dev server, per the earlier `.next`-corruption lesson in this log) both clean.
